...
def is_toy_dataset(opt):
    return opt.problem_name in ['gmm','checkerboard', 'moon-to-spiral','semicircle', 
    'RNAsc','navi','petal','gaussian2', 'RNA5dim', 'hesc', 'eb5', 'cite5', 'multi5', 'cite100', 'multi100']
...
def save_checkpoint(opt, runner, keys, stage_it):
    checkpoint = {}
    fn = opt.ckpt_path + "/stage_{0}_LOO_{1}_seed_{2}.npz".format(
        stage_it, opt.LOO, opt.seed
    )
...
def load_model(checkpoint, device=None, if_mnist=False):
    ...
    try:
        cfg = chkpnt["cfg"]
        if if_mnist:
            model = VAE(z_dim=cfg.z_dim, beta=cfg.beta, x_std=cfg.x_std, nc=1, if_mnist=if_mnist)
        else:
            model = VAE(z_dim=cfg.z_dim, beta=cfg.beta, x_std=cfg.x_std)
    except:
        if if_mnist:
            model = VAE(z_dim=256, beta=0.1, x_std=0.1, nc=1, if_mnist=if_mnist)
        else:
            model = VAE(z_dim=256)
