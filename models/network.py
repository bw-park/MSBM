...
class UNetVectorField(nn.Module):
    def __init__(self, opt, cfg, timesteps=1000):
    ...
    def forward(self, x, t) -> torch.Tensor:
        ...
        B, D = x.shape
        assert t.shape == (B,)
        assert D == 3 * 64 * 64 or D == 1 * 32 * 32

        batch = {}
        if self.problem_name == "afhq":
            assert D == 3 * 64 * 64
            batch["noisy_x"] = x.reshape(B, 3, 64, 64)
        elif self.problem_name == "mnist": 
            assert D == 1 * 32 * 32
            batch["noisy_x"] = x.reshape(B, 1, 32, 32)
